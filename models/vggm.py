...
class VGGNet(DebugModule):
    ...
    def __init__(self, n_channels_in, layers):
        # n_channels_in : 입력하는 채널 수
        # layers : 각 layer의 매개변수를 정의하는 dictionary list
        ...
        for l_id, lr in enumerate(self.layers):
            ...
            conv_type = self.conv_dict[lr['type']] # 합성곱 레이어
            norm_type = self.norm_dict[lr['type']] # normalization layer
            ...
            if 'maxpool' in lr: # max pool이면, pooling layer를 추가
    ...
    def forward(self, inp):
        ...
        self.debug_line('Input', inp) # 입력 텐서 inp를 debug_line method를 통해 기록
        out = inp # out에 할당
        for l_id, lr in enumerate(self.layers):
            l_id += 1
            name = 'fc' if 'fc' in lr['type'] else 'conv'
            ...
            if 'maxpool' in lr: # layer 설정에 maxpool이 있으면 해당 풀링 레이어를 적용
...
class SyncnetMFCC(SyncnetBase):
    # audio-video 동기화 신경망 모델
    def create_net_aud(self):
        # audio data를 처리하는 network
        layers = [
